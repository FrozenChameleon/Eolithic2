...
AHFloat CircularEaseInOut(AHFloat p)
{
	if(p < 0.5)
	{
		return 0.5 * (1 - Math_sqrt(1 - 4 * (p * p)));
	}
	else
	{
		return 0.5 * (Math_sqrt(-((2 * p) - 3) * ((2 * p) - 1)) + 1);
	}
}
...
{
	return (p == 0.0) ? p : Math_pow(2, 10 * (p - 1));
}

// Modeled after the exponential function y = -2^(-10x) + 1
AHFloat ExponentialEaseOut(AHFloat p)
{
	return (p == 1.0) ? p : 1 - Math_pow(2, -10 * p);
}
...
AHFloat ExponentialEaseInOut(AHFloat p)
{
	...
	if(p < 0.5)
	{
		return 0.5 * Math_pow(2, (20 * p) - 10);
	}
	else
	{
		return -0.5 * Math_pow(2, (-20 * p) + 10) + 1;
	}
}
...
{
	return Math_sin(13 * MATH_HALFPI * p) * Math_pow(2, 10 * (p - 1));
}

// Modeled after the damped sine wave y = sin(-13pi/2*(x + 1))*pow(2, -10x) + 1
AHFloat ElasticEaseOut(AHFloat p)
{
	return Math_sin(-13 * MATH_HALFPI * (p + 1)) * Math_pow(2, -10 * p) + 1;
}
...
AHFloat ElasticEaseInOut(AHFloat p)
{
	if(p < 0.5)
	{
		return 0.5 * Math_sin(13 * MATH_HALFPI * (2 * p)) * Math_pow(2, 10 * ((2 * p) - 1));
	}
	else
	{
		return 0.5 * (Math_sin(-13 * MATH_HALFPI * ((2 * p - 1) + 1)) * Math_pow(2, -10 * (2 * p - 1)) + 2);
	}
}
...
{
	return p * p * p - p * Math_sin(p * MATH_PI);
}
...
{
	AHFloat f = (1 - p);
	return 1 - (f * f * f - f * Math_sin(f * MATH_PI));
}
...
AHFloat BackEaseInOut(AHFloat p)
{
	...
	{
		AHFloat f = 2 * p;
		return 0.5 * (f * f * f - f * Math_sin(f * MATH_PI));
	}
	else
	{
		AHFloat f = (1 - (2*p - 1));
		return 0.5 * (1 - (f * f * f - f * Math_sin(f * MATH_PI))) + 0.5;
	}
}
